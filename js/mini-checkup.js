...
(function () {
  ...
  // ── SUMMARY TEMPLATES ─────────────────────────────
  // Build a per-layer observation from the user's answers
  function buildLayerObservation(layer, answers) {
    const a = (answers[layer.id] || []).join(' ').toLowerCase();
    const obs = {
      strategy: () => {
        if (/×2|x2|удво|масштаб|выручк|оборот/.test(a)) return 'Цель роста есть, но без явных рычагов рост превращается в гонку без приборов. Чекап 9k показывает, какие 3 рычага дают 80% результата.';
        if (/маржа|прибыл|рентабельн/.test(a)) return 'Цель по марже — самая частая и самая сложная: без видимости маржи по проектам её невозможно растить осмысленно.';
        if (/команд|найм|выйти|делегир/.test(a)) return 'Выход из операционки — это не «нанять человека», это «выстроить систему, в которой человек справится». EDL OS как раз закрывает эту часть.';
        return 'Цель есть, но не привязана к измеримой метрике. Первый шаг — превратить её в 3 ключевые ставки на 12 месяцев.';
      },
      funnel: () => {
        if (/сарафан|рекоменд/.test(a)) return 'Сарафан — отличный канал на старте, но он непредсказуем и зависит от 1-2 источников. Нужен второй платный канал, чтобы спать спокойно.';
        if (/реклам|таргет|директ|яндекс|google/.test(a)) return 'С платным трафиком главное — видеть LTV по каналу, а не только CAC. Иначе масштабирование уходит в минус.';
        if (/холодн|тендер|b2b|кп/.test(a)) return 'В B2B-цикле побеждает тот, кто видит pipeline по неделям и знает, какие сделки «зависли». Эту картину EDL собирает каждое утро.';
        return 'Воронка без чисел — это вера. С цифрами появляется управление.';
      },
      ops: () => {
        if (/сам|один|тяну/.test(a)) return 'Когда основатель — единственная критическая точка, бизнес не масштабируется и не продаётся. EDL начинает с того, чтобы вынуть вас из 2-3 ежедневных решений.';
        if (/перегруж|выгора|тян|80/.test(a)) return 'Если 80% работы на 20% людей — это потеря 2 ключевых сотрудников от выгорания. EDL подсвечивает дисбаланс за 2-3 недели до того, как он становится катастрофой.';
        if (/текучк|увольн|найм/.test(a)) return 'Текучка — это симптом, не диагноз. EDL показывает, какие проекты систематически перегружают команду и где включается выгорание.';
        if (/процесс|регламент|руками|excel/.test(a)) return 'То, что повторяется руками каждую неделю, можно автоматизировать на 2-5 дней работы и сэкономить 5-15 часов в неделю.';
        return 'Скрытая загрузка команды — самый недооценённый риск. Видимость даёт +20-30% к производительности без найма.';
      },
      money: () => {
        if (/excel|таблиц|руками/.test(a)) return 'Excel-учёт работает до 20 клиентов. После — расхождения, ошибки округления, скрытые убыточные клиенты. Видимость нужна не для бухгалтерии, а для собственника.';
        if (/1с|бухгалт/.test(a)) return '1С показывает учёт, а не экономику. EDL показывает реальную маржу по контракту в день закрытия, а не в конце квартала.';
        if (/не счита|никак|на глаз/.test(a)) return 'Если маржа считается «на глаз», в портфеле почти всегда есть 2-3 убыточных клиента, которые съедают прибыль. Их видно за 1 чекап.';
        if (/ндс|налог|антидробл|22/.test(a)) return 'НДС 22% и антидробление 2026 — это переоценка маржи по каждому клиенту. Часть клиентов теперь нерентабельны — их нужно увидеть.';
        if (/кассов|разрыв|кэш/.test(a)) return 'Кассовый разрыв виден за 3 недели до факта, если есть прогноз. Без прогноза — узнаёте в день, когда нужно платить зарплату.';
        return 'Маржа с точностью до проекта — это разница между управлением и догадками. Чекап даёт картинку за 48 часов.';
      },
      bonus: () => {
        if (/переключ|систем|вкладк/.test(a)) return 'Это и есть главное обещание EDL OS — одна сводка в Telegram вместо 10 систем.';
        if (/никто|не видит/.test(a)) return 'Когда цифры только у вас в голове — бизнес не делегируется. EDL делает их общим языком для команды.';
        if (/делаю работу/.test(a)) return 'Работа «за сотрудников» — следствие отсутствия процесса и видимости. EDL начинает с диагноза, какие 2-3 регламента дадут максимум.';
        return null;
      }
    };
  ...
  function pickRecommendations(answers) {
    ...
    const recs = [];
    if (/×2|удво|масштаб|выручк/.test(all)) recs.push('Зафиксировать 3 ключевые ставки на 12 месяцев и привязать к ним недельные метрики.');
    if (/маржа|прибыл|рентабельн/.test(all) || /excel|1с|не счита/.test(all)) recs.push('Построить отчёт «маржа по клиенту» — он подсветит 2-3 нерентабельных контракта.');
    if (/сарафан|рекоменд/.test(all)) recs.push('Запустить второй канал привлечения, чтобы снять риск концентрации.');
    if (/перегруж|выгора|тян|сам/.test(all)) recs.push('Найти 2-3 процесса, которые забирают у вас > 5 часов в неделю, и автоматизировать их.');
    if (/ндс|налог|антидробл/.test(all)) recs.push('Пересчитать маржу по клиентам после НДС 22% — часть пойдёт под пересмотр цен.');
    if (/кассов|разрыв|кэш/.test(all)) recs.push('Внедрить прогноз cash-flow на 30/60/90 дней.');
    if (recs.length === 0) recs.push(
      'Зафиксировать стартовую картину: маржа, нагрузка, воронка — 1 экран на 1 страницу.',
  ...
  async function finalize() {
    ...
    await appendAiMsg(body,
      'Готово. Я прошёл по 4 слоям Founder OS — Стратегия · Воронка · Операционка · Деньги. Собираю короткий разбор...',
      { delay: 500 }
    );
    await sleep(REDUCED_MOTION ? 0 : 800);
    renderSummary(buildSummary(state.answers));
    // Hide composer
  ...
  function renderSummary(summary) {
    ...
    summaryEl.className = 'mc-summary mc-msg';
    summaryEl.innerHTML = `
      <p class="mc-summary__intro">Это <strong>предварительная картина</strong> на основе ваших ответов. Полный разбор с конкретными цифрами по вашему бизнесу — в Бизнес-чекапе.</p>
      <div class="mc-summary__layers">
        ${summary.layers.map(l => `
          <div class="mc-layer-card">
            <div class="mc-layer-card__head">
              <span class="mc-layer-card__icon">${l.icon}</span>
              <span>${l.label}</span>
            </div>
            <div class="mc-layer-card__body">
              ${l.heard ? `<em>Услышал:</em> «${escapeHtml(truncate(l.heard, 110))}»` : ''}
              ${l.observation ? `<div class="mc-layer-card__obs">${l.observation}</div>` : ''}
            </div>
          </div>
        `).join('')}
      </div>
      <div class="mc-summary__recs">
        <h4>Что я бы сделал в первую очередь:</h4>
        <ol>
          ${summary.recs.map(r => `<li>${r}</li>`).join('')}
        </ol>
      </div>
      ${summary.bonusObs ? `<div class="mc-layer-card__obs" style="background:var(--color-gray-100); margin-bottom:20px;"><strong>P.S.</strong> ${summary.bonusObs}</div>` : ''}
      <div class="mc-summary__cta-block">
        <div class="mc-summary__cta-text">
          Хотите увидеть это <strong>с цифрами по вашему бизнесу</strong>?<br>
          Бизнес-чекап за 9 000 ₽ — PDF-разбор + майнд-карта в FigJam за 48 часов.<br>
          <strong>Безусловный возврат 14 дней.</strong>
        </div>
        <a href="audit.html" class="mc-summary__cta-btn" data-cta-type="audit" data-cta-location="mini_checkup">Перейти к Бизнес-чекапу 9 000 ₽ →</a>
        <a href="https://t.me/edl_os" target="_blank" rel="noopener" class="mc-summary__secondary">Не готов(а) сейчас — подпишусь на Telegram-канал</a>
    ...
    // Email form (frontend-only stub; saves to localStorage + analytics)
    ...
    // Track CTA clicks inside summary
    summaryEl.querySelectorAll('[data-cta-type]').forEach(a => {
