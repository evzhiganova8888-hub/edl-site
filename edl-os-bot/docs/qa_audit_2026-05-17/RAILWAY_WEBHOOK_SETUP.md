...
>
> ⚠️ **Обновлено 17.05.2026 после первой попытки**. Первая активация дала
> 502 Bad Gateway (см. история ниже + `improvement_plan.md` §1). Исправлено
> в коде: webhook handler теперь не блокирует ответ Telegram (кладёт update
> в очередь и отдаёт 200 OK мгновенно). Эта инструкция актуальна **после**
> мерджа PR с фиксом.
