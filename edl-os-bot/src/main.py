...
# Global PTB application (shared between FastAPI lifespan and polling task)
...
async def _run_polling(app: Application) -> None:
    """Запуск polling в фоне для dev-режима."""
    try:
...
api = FastAPI(title="EDL OS Bot", version="0.1.0", lifespan=lifespan)


@api.get("/health")
async def health() -> dict:
    return {"status": "ok", "use_webhook": settings.use_webhook}
