...
def offer_summary() -> str:
    ...
    return (
        ...
        "• Plus — 14 000 ₽, 7 рабочих дней (с видео-разбором от команды EDL OS).\n\n"
        f"{payment_line}\n\n"
        "Возврат 100% оплаты в течение 14 календарных дней с момента передачи "
        "материалов — *условный*. Возвращаем оплату при выполнении ОБОИХ условий "
        "одновременно: (1) ответы клиента в Чекапе соответствуют рубрике "
        "качества; (2) ни одну из рекомендаций отчёта клиент не может реализовать "
        "в своей компании. Контекст для реалистичности рекомендаций собирается "
        "во время Чекапа. Запрос — /refund.\n\n"
